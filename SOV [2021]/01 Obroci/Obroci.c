...
void init_list(OBROCI **);
OBROCI *create_new_item(const char *, const char *, const unsigned, const unsigned);
void destroy_list(OBROCI **);

void add_to_list(OBROCI **, OBROCI *);
void add_to_list_sorted(OBROCI **, OBROCI *);
void add_to_list_begin(OBROCI **, OBROCI *);

FILE *s_fopen(char *, char *);
void data_load(FILE *, OBROCI **);

void print_list(OBROCI *, char *);
void najmanje_kalorican(OBROCI *, char *);

void printf_list(OBROCI *);
...
void add_to_list_sorted(OBROCI **glava, OBROCI *novi)
{
	if(*glava == NULL || (*glava) -> gramaza >= novi -> gramaza) {
		novi -> sledeci = *glava;
		*glava = novi;
	}
	else if( (*glava) -> sledeci != NULL && (*glava) -> gramaza < novi -> gramaza)
	{
		add_to_list_sorted(&(*glava) -> sledeci, novi);
	}
	else
	{
...
void add_to_list_begin(OBROCI **glava, OBROCI *novi)
{
	if(*glava == NULL)
		*glava = novi;
	else 
...
void printf_list(OBROCI *g)
{
	while(g != NULL) {
		printf("%-20s %-20s %5u %5u\n", g -> naziv, g -> vrsta, g -> gramaza, g -> kalorije);
		g = g -> sledeci;
	}
